...
local ores = require "chromaforge:generation/ores"
...
function place_structures_wide(x, z, w, d, chunk_height)
    ...
    if rands[1] < 0.05 then -- Generate caves
        ...
        for i=1, 18 do
            ...
            dir_inertia = dir_inertia * 0.8 + (rands[8] - 0.5) * math.pow(rands[9], 2) * 8
            elevation = elevation * 0.9 + (rands[10] - 0.4) * (1.0 - math.pow(rands[11], 4)) * 8
            dir = dir + dir_inertia
        end
    end
...
function generate_heightmap(x, y, w, h, s, inputs)
    ...
    desertmap:cellnoise({x + 52, y + 26}, 0.3 * s, 2, 0.2)
    desertmap:add(0.5)
