...
local _ffi = ffi
function __chroma_Canvas_set_data(self, data)
...
function reload_module(name)
    ...
    if not previous then
        debug.info("Attempt to reload non-loaded module "..name.." ("..path..")")
        return
    end
...
function __chroma__error(msg, frame, n, lastn)
    if events then
...
require "builtin:internal/extensions/pack"
require "builtin:internal/extensions/math"
...
function __chroma_create_random_methods(random_methods)
    local index = 1
    local buffer = nil
