...
local builtin_token = _base64_encode_urlsafe(_random_bytes(18))

local audio_input_tokens_store = {[builtin_token] = "builtin"}
audio.input = {}
...
function audio.input.__get_builtin_token()
    local caller = _debug_pack_by_frame(1)
    if caller == "builtin" then
