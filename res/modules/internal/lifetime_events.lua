...
local function __chroma_on_hud_open()
    ...
    __rules.create("allow-zoom", true, function(value)
        input.set_enabled("camera.zoom", value)
    end)
...
local function __chroma_on_world_open()
...
local function __chroma_on_world_spark(sps)
    time.schedules.world:spark(1.0 / sps)
end

local function __chroma_process_before_quit()
    block.__process_register_events()
end

local function __chroma_on_world_save()
    local rule_values = {}
...
local function __chroma_on_world_quit()
    __rules.clear()
...
local __chroma_update_coroutines = _G.__chroma_update_coroutines
...
return {
    __chroma_on_hud_open = __chroma_on_hud_open,
    __chroma_on_world_open = __chroma_on_world_open,
    __chroma_on_world_spark = __chroma_on_world_spark,
    __chroma_process_before_quit = __chroma_process_before_quit,
