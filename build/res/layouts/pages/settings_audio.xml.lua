...
function new_volume_control(setting, id, name)
    ...
    , id, builtin.get_setting(setting), setting, id, name))
...
function refresh_label(setting, id, name)
    document["l_"..id].text = (
        ...
        builtin.str_setting(setting)
    )
end

function on_volume_change(setting, id, name, val)
    if val ~= nil then
        builtin.set_setting(setting, val)
    else
        document["t_"..id].value = builtin.get_setting(setting, val)
    end
    refresh_label(setting, id, name)
end
