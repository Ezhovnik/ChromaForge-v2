...
CFModel позволяет описывать [скелет](rigging.md) модели. Для этого используется элемент `@bone` (кость).
...
В списке `пак/preload.json` при указании параметра `"squash": true` cfmodel будет загружен как единая модель, без скелета.
