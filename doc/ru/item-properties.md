...
- `none` - невидимый тип, используется только для `builtin:empty` (пустой предмет). Не влияет на появление предмета на панель доступа к контенту. Тип может быть удалён в будущем
...
- `block` - отображает предпросмотр блока. В icon указывается строковый id блока который нужно отображать. Пример `chromaforge:wood`
...
```json
"placing-block": "chromaforge:bedrock"
```
