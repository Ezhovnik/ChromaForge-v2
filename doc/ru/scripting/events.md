...
Вызывается при нажатии на блок `player.build` (по умолчанию `ПКМ`). Предотвращает установку блоков, если возвращает `true`
...
> при неосторожном или чрезмерном использовании могут привести к проблемам с производительностью.
...
При малой нагрузке происходит в течение первого такта блока (spark-interval).
До вызова события `on_block_spark` не вызывается.
...
Вызывается при нажатии `player.build` (по умолчанию `ПКМ`) не на блок.

```lua
function on_use_on_block(x: int, y: int, z: int, playerid: int, normal: vec3)
```

Вызывается при нажатии `player.build` (по умолчанию `ПКМ`) на блок. Предотвращает установку блока, прописанного в `placing-block` если возвращает `true`

```lua
function on_block_break_by(x: int, y: int, z: int, playerid: int)
```

Вызывается при нажатии `player.attack` (по умолчанию `ЛКМ`) на блок (в т.ч неразрушимый). Предотвращает разрушение блока, если возвращает `true`
...
Удаляет все события с префиксом `packid:`. Вы выходе из мира выгружаются события всех паков, включая `builtin:`.
