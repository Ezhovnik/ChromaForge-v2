...
```lua
local pathfinding = entity:require_component("builtin:pathfinding")
...
-- (следует использовать компонент builtin:mob - функция mob.follow_waypoints)
...
```lua
local mob = entity:require_component("builtin:mob")
...
-- Если не указан pathfinding, требуется наличие у сущности компонента builtin:pathfinding
