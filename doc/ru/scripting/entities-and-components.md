...
Пример: `builtin:scripts/components/pathfinding.lua` -> `builtin:pathfinding`.
...
Вызывается при атаке на сущность (`player.attack` (по умолчанию `ЛКМ`) по сущности). Первым аргументом передается UID атакующей сущности. ID атакующего игрока передается вторым аргументом. Если сущность была атакована не игроком, значение второго аргумента будет равно -1.

```lua
function on_used(playerid: int)
```

Вызывается при использовании сущности (`player.build` (по умолчанию `ПКМ`) по сущности). ID игрока передается в качестве аргумента.
