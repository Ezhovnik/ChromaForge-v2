...
- окончание `.item` добавляется только для замены сгенерированного для блока предмета. Пример: `chromaforge:stone.item` - предмет сгенерированный для блока камня.
