...
- [Генератор мира](#генератор-мира)
  ...
  - [Ручная расстановка структур](#ручная-расстановка-структур)
    - [Размещения структур / тоннелей / одиночных блоков](#размещения-структур--тоннелей--одиночных-блоков)
      - [Структура](#структура)
      - [Тоннель](#тоннель)
      - [Одиночный блок](#одиночный-блок)
  ...
  - [Генератор 'Standart' (chromaforge:standart)](#генератор-standart-chromaforgestandart)
    - [Добавление новой руды](#добавление-новой-руды)
...
Разберем структуру биома на примере леса из генератора chromaforge:standart:
...
layers = [
    {below-sea-level=false, height=1, block="chromaforge:grass_block"},
    {below-sea-level=false, height=7, block="chromaforge:dirt"},
    {height=-1, block="chromaforge:stone"},
    {height=1, block="chromaforge:bazalt"}
]
sea-layers = [
    {height=-1, block="chromaforge:water"}
]
plant-chance = 0.4
plants = [
    {weight=1, block="chromaforge:grass"},
    {weight=0.03, block="chromaforge:flower"}
]
...
### Размещения структур / тоннелей / одиночных блоков

Размещение структуры / линии / одиночного блока представляет собой массив из заданного
набора параметров.
...
`builtin:struct_air` - блок, которые следует использовать в фрагментах для обозначения пустого пространства, которое не должно заполняться блоками при генерации в мире.

<image src="../../res/textures/blocks/struct_air.png" width="128px" height="128px" style="image-rendering: pixelated">

## Генератор 'Standart' (chromaforge:standart)

### Добавление новой руды

Чтобы добавить новую руду из своего пака:

1. В папке `generators` создайте папку `standart.files` (standart.toml создавать не нужно).
2. В созданной папке создайте папку fragments и поместите в неё файл фрагмента руды.
3. В `standart.files` создайте файл structures.toml:

```toml
имя_фрагмента = {}
```

4. Также в `standart.files` создайте файл ores.json:
