...
- "cube" - используется по-умолчанию для всех обычных блоков
- "none" - невидимый блок (пример: воздух)
- "X" - модель травы (крест из двух спрайтов)
- "aabb" - модель, соответствующая хитбоксу блока (составной хитбокс будет объединен в один). Примеры: факела, лампочки, панели.
...
Пример: [пользовательские свойства пака **chromaforge**](../../res/content/chromaforge/config/user-props.toml).

## Свойства, вводимые паком `chromaforge`

### Прочность - *chromaforge:durability*

Время разрушения блока без инструментов и эффектов в секундах.

### Лут - *chromaforge:loot*
...
Пример: `chromaforge:dirt.item`.

Для генерации лута следует использовать функцию `block_loot(block_id: int)` в модуле `chromaforge:util`.
...
### Интервал искр (тактов) - *spark-interval*

Интервал в искрах мира (1/20 секуды). Значение 20 приводит к интервалу вызова on_block_spark равному одной секунде.
