...
void TextsRenderer::renderNote(
    ...
    if (preset.displayMode == NoteDisplayMode::Projected) {
        ...
        if (glm::abs(preset.perspective) > 0.0001f) {
            float scale2 = scale / (glm::distance(camera.position, pos) * camera.zoom * glm::sqrt(glm::tan(camera.getFov() * 0.5f)));
        ...
        if (frontLayer) {
            ...
            pos = screenPos / screenPos.w;
                    ...
                    pos + xvec * (width * 0.5f) * preset.scale + yvec * static_cast<float>(height) * preset.scale
                )
    ) {
        return;
    }
