...
void ParticlesRenderer::renderParticle(
    ...
    auto& preset = emitter.preset;
    float scale = 
        1.0f + ((particle.random ^ 2628172) % 1000) * 0.001f * preset.sizeSpread;
    ...
    if (glm::abs(angle) >= 0.005f) {
        ...
        localRight = 
            right * rotatedRight.x +
            localUp * rotatedRight.y +
...
void ParticlesRenderer::update(const Camera& camera, float deltaTime) {
