...
void ModelBatch::draw(
    ...
    for (size_t i = 0; i < vcount / 3; ++i) {
        if (index + MB_VERTEX_SIZE * 3 > capacity * MB_VERTEX_SIZE) {
            flush();
        }
...
void ModelBatch::draw(const model::Model* model, const texture_names_map* varTextures) {
...
void ModelBatch::box(glm::vec3 pos, glm::vec3 size, glm::vec4 lights) {
    if (index + 36 < capacity * MB_VERTEX_SIZE) flush();
...
void ModelBatch::flush() {
    ...
    if (texture == nullptr) texture = blank.get();
    texture->bind();
    mesh->reload(buffer.get(), index / MB_VERTEX_SIZE);
    mesh->draw();
    index = 0;
}
