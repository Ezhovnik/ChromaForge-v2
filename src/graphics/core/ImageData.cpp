...
ImageData::ImageData(ImageFormat format, uint width, uint height, const ubyte* data) : format(format), width(width), height(height) {
    size_t pixsize;
    switch (format) {
        case ImageFormat::rgb888: pixsize = 3; break;
        case ImageFormat::rgba8888: pixsize = 4; break;
        ...
        case ImageFormat::rgba8888: {
            ...
            for (uint y = 0; y < height / 2; ++y) {
                for (uint x = 0; x < width; ++x) {
                    for (uint c = 0; c < size; ++c) {
                        ubyte temp = data[(y * width + x) * size + c];
                        data[(y * width + x) * size + c] = data[((height - y - 1) * width + x) * size + c];
...
void ImageData::blit(const ImageData& image, int x, int y) {
    ...
    if (format == ImageFormat::rgba8888 && image.format == ImageFormat::rgb888) {
...
void ImageData::blitRGB_on_RGBA(const ImageData& image, int x, int y) {
    ...
    for (uint srcy = std::max(0, -y); srcy < std::min(srcheight, height - y); ++srcy) {
        for (uint srcx = std::max(0, -x); srcx < std::min(srcwidth, width - x); ++srcx) {
            uint dstx = srcx + x;
            uint dsty = srcy + y;
...
void ImageData::blitMatchingFormat(const ImageData& image, int x, int y) {
    ...
    for (uint srcy = std::max(0, -y); srcy < std::min(src_height, height - y); ++srcy) {
        for (uint srcx = std::max(0, -x); srcx < std::min(src_width, width - x); ++srcx) {
            uint dstx = srcx + x;
            uint dsty = srcy + y;
...
void ImageData::extrude(int x, int y, int w, int h) {
    ...
    if (y > 0 && static_cast<uint>(y) < height) {
        for (uint ex = std::max(x, 0); static_cast<uint>(ex) < x + w; ++ex) {
...
void ImageData::fixAlphaColor() {
    assert(format == ImageFormat::rgba8888);
    ...
    for (int i = 0; i < 3; ++i) sums[i] /= samples;
    for (uint ly = 0; ly < height; ++ly) {
        for (uint lx = 0; lx < width; ++lx) {
            if (data[(ly * width + lx) * 4 + 3] != 0) continue;
            for (int i = 0; i < 3; ++i) {
...
ImageData* toRGBA(ImageData* image){
    ...
    if (format == ImageFormat::rgba8888) {
        ...
        std::memcpy(newData, data, width * height * 4);
        return new ImageData(ImageFormat::rgba8888, width, height, newData);
    }
    else if (format == ImageFormat::rgb888) {
        const ubyte* src = static_cast<const ubyte*>(data);
        ubyte* dst = new ubyte[width * height * 4];
