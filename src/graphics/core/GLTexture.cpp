...
GLTexture::GLTexture(uint id, uint width, uint height) : Texture(width, height), id(id) {
}
...
void GLTexture::reload(const ubyte* data) {
    glBindTexture(GL_TEXTURE_2D, id);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, static_cast<const GLvoid*>(data));
    glBindTexture(GL_TEXTURE_2D, 0);
}
...
std::unique_ptr<ImageData> GLTexture::readData() {
    ...
    glBindTexture(GL_TEXTURE_2D, 0);
    return std::make_unique<ImageData>(ImageFormat::rgba8888, width, height, std::move(data));
}
