...
void TextureAnimator::update(float delta) {
    ...
    for (auto& elem : animations) {
        ...
        if (frameNum != elem.currentFrame) {
            ...
            if (changedTextures.find(elemDstId) == changedTextures.end()) changedTextures.insert(elemDstId);
            ...
            float srcPosY = elem.srcTexture->getHeight() - frame.size.y - frame.srcPos.y;
