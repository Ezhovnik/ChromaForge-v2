...
static void _readContainer(
    ...
    for (auto& sub : element->getElements()) {
        ...
        if (subnode) container.add(subnode);
    }
}
...
static std::wstring readAndProcessInnerText(
    const xml::xmlelement& element,
...
static std::shared_ptr<UINode> readContainer(
    UIXmlReader& reader,
    const xml::xmlelement& element
) {
...
static std::shared_ptr<UINode> readButton(
    UIXmlReader& reader,
    const xml::xmlelement& element
) {
...
static std::shared_ptr<UINode> readTextBox(
    UIXmlReader& reader,
    const xml::xmlelement& element
) {
...
static std::shared_ptr<UINode> readCheckBox(
    UIXmlReader& reader,
    const xml::xmlelement& element
) {
    auto text = readAndProcessInnerText(element, reader.getContext());
...
static std::shared_ptr<UINode> readTrackBar(
    ...
    if (element->has("sub-consumer")) {
        bar->setSubConsumer(scripting::create_number_consumer(env, element->attr("sub-consumer").getText(), file));
    }
    if (element->has("track-color")) bar->setTrackColor(element->attr("track-color").asColor());
    if (element->has("change-on-release")) bar->setChangeOnRelease(element->attr("change-on-release").asBool());

    return bar;
}

static std::shared_ptr<UINode> readInputBindBox(
    UIXmlReader& reader,
    const xml::xmlelement& element
) {
...
static std::shared_ptr<UINode> readInventory(
    UIXmlReader& reader,
    const xml::xmlelement& element
) {
...
static std::shared_ptr<UINode> readPageBox(
    UIXmlReader& reader,
    const xml::xmlelement& element
) {
