...
static util::TextureRegion determine_texture_region(
    ...
    if (texture.length() < 2 || texture.at(0) != '$') {
        return util::get_texture_region(assets, texture, "blocks:" + TEXTURE_NOTFOUND);
    }

    ubyte sideIndex = texture.at(1) - '0';
    ...
    return util::get_texture_region(assets, "blocks:" + TEXTURE_NOTFOUND, "");
}
