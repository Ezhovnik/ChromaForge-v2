...
SlotView::SlotView(SlotLayout layout) : UINode(glm::vec2(), glm::vec2(InventoryView::SLOT_SIZE)), layout(layout) {
...
void SlotView::draw(const GfxContext* parent_context, Assets* assets) {
    ...
    switch (item->iconType) {
        ...
        case ItemIconType::Block: {
            const Block& chosen_block = content->requireBlock(item->icon);
...
void SlotView::bind(ItemStack& stack, LevelFrontend* frontend, InventoryInteraction* interaction) {
    bound = &stack;
...
void InventoryView::bind(std::shared_ptr<Inventory> inventory, LevelFrontend* frontend, InventoryInteraction* interaction) {
    ...
    for (auto slot : slots) {
        slot->bind(inventory->getSlot(slot->getLayout().index), frontend, interaction);
    }
}
