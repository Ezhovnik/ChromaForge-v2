...
void HudRenderer::draw(Level* level){
	...
	Player* player = level->player;
	{
		...
		if (choosen_block->model == Block_models::CUBE){
			batch->blockSprite(width/2-24, uicamera->fov - 72, 48, 48, 16, choosen_block->textureFaces, glm::vec4(1.0f));
		} else if (choosen_block->model == Block_models::X){
			batch->sprite(width/2-24, uicamera->fov - 72, 48, 48, 16, choosen_block->textureFaces[3], glm::vec4(1.0f));
		}
	}
