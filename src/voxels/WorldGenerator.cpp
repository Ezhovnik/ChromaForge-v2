...
int generate_tree(fnl_state *noise, PseudoRandom* random, const float* heights, int real_x, int real_y, int real_z, int tileSize){
	...
	if (lx == 0 && lz == 0 && real_y - height < 4*radius) return 6;
	if (lx*lx+ly*ly/2+lz*lz < radius*radius) return 7;
	return 0;
}
	...
	for (int z = 0; z < CHUNK_DEPTH; z++){
		...
		for (int x = 0; x < CHUNK_WIDTH; x++){
			...
			for (int y = 0; y < CHUNK_HEIGHT; y++){
				int real_y = y + cy * CHUNK_HEIGHT;
				int id = real_y < 55 ? 8 : 0; 
