...
void ChunksLoader::_thread(){
	...
	while (working){
		...
		Chunk* chunk = current;
		chunks._setOffset(chunk->chunk_x - 1, chunk->chunk_y - 1, chunk->chunk_z - 1);
		for (size_t i = 0; i < CLOSES_CNT; ++i){
		...
		lighting.onChunkLoaded(chunk->chunk_x, chunk->chunk_y, chunk->chunk_z, true);
		chunks.clear(false);
...
void ChunksLoader::perform(Chunk* chunk, Chunk** closes_passed){
	...
	chunk->incref();
	if (closes == nullptr) closes = new Chunk*[CLOSES_CNT];
	for (int i = 0; i < CLOSES_CNT; ++i){
		Chunk* other = closes_passed[i];
		if (other == nullptr) closes[i] = nullptr;
		else {
			other->incref();
			closes[i] = other;
		}
