...
RayRelation Ray::intersectAAFace<AAFaceKind::Xperp>( 
	...
	if (fabs(glm::dot(dir, X_AXIS)) < 1.0E-8) return RayRelation::Parallel;

	scalar_t rayCoef = (faceMin.x - origin.x) / (dir.x);// equivalent to distance if dir normalized
	if (rayCoef < 0) return RayRelation::None;
	rayvec3 intersectPoint = {faceMin.x, rayCoef * dir.y + origin.y, rayCoef * dir.z + origin.z};
...
RayRelation Ray::intersectAAFace<AAFaceKind::Yperp>(
				   ...
				   glm::ivec3& normal_ret,
				   scalar_t& distance_ret){
	if (fabs(glm::dot(dir, Y_AXIS)) < 1.0E-8) return RayRelation::Parallel;

	scalar_t rayCoef = (faceMin.y - origin.y) / (dir.y);// equivalent to distance if dir normalized
	if (rayCoef < 0) return RayRelation::None;
	rayvec3 intersectPoint = {rayCoef * dir.x + origin.x, faceMin.y, rayCoef * dir.z + origin.z};
...
RayRelation Ray::intersectAAFace<AAFaceKind::Zperp>(
				   ...
				   glm::ivec3& normal_ret,
				   scalar_t& distance_ret){
	if (fabs(glm::dot(dir, Z_AXIS)) < 1.0E-8) return RayRelation::Parallel;

	scalar_t rayCoef = (faceMin.z - origin.z) / (dir.z); // equivalent to distance if dir normalized
	if (rayCoef < 0) return RayRelation::None;
	rayvec3 intersectPoint = {rayCoef * dir.x + origin.x, rayCoef * dir.y + origin.y, faceMin.z};
...
RayRelation Ray::intersectAABBFaces(
	...
	if (intersectAAFace<AABBFaces::KINDS_ORDER[0]>(boxFaces.faces[0].first, boxFaces.faces[0].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectAAFace<AABBFaces::KINDS_ORDER[1]>(boxFaces.faces[1].first, boxFaces.faces[1].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectAAFace<AABBFaces::KINDS_ORDER[2]>(boxFaces.faces[2].first, boxFaces.faces[2].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectAAFace<AABBFaces::KINDS_ORDER[3]>(boxFaces.faces[3].first, boxFaces.faces[3].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectAAFace<AABBFaces::KINDS_ORDER[4]>(boxFaces.faces[4].first, boxFaces.faces[4].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectAAFace<AABBFaces::KINDS_ORDER[5]>(boxFaces.faces[5].first, boxFaces.faces[5].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}
