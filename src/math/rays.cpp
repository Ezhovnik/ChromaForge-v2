...
RayRelation Rays::rayIntersectAAFace<AAFaceKind::Xperp>( 
	...
	if (fabs(glm::dot(rayDir, X_AXIS)) < 1.0E-8) return RayRelation::Parallel;
	...
	rayvec3 intersectPoint = {faceMin.x, rayCoef * rayDir.y + rayOrigin.y, rayCoef * rayDir.z + rayOrigin.z};
...
RayRelation Rays::rayIntersectAAFace<AAFaceKind::Yperp>(
				   ...
				   glm::ivec3& normal_ret,
				   scalar_t& distance_ret){
	if (fabs(glm::dot(rayDir, Y_AXIS)) < 1.0E-8) return RayRelation::Parallel;

	scalar_t rayCoef = (faceMin.y - rayOrigin.y) / (rayDir.y);// equivalent to distance if raydir normalized
	if (rayCoef < 0) return RayRelation::None;
	rayvec3 intersectPoint = {rayCoef * rayDir.x + rayOrigin.x, faceMin.y, rayCoef * rayDir.z + rayOrigin.z};
...
RayRelation Rays::rayIntersectAAFace<AAFaceKind::Zperp>(
				   ...
				   glm::ivec3& normal_ret,
				   scalar_t& distance_ret){
	if (fabs(glm::dot(rayDir, Z_AXIS)) < 1.0E-8) return RayRelation::Parallel;

	scalar_t rayCoef = (faceMin.z - rayOrigin.z) / (rayDir.z); // equivalent to distance if raydir normalized
	if (rayCoef < 0) return RayRelation::None;
	rayvec3 intersectPoint = {rayCoef * rayDir.x + rayOrigin.x, rayCoef * rayDir.y + rayOrigin.y, faceMin.z};
...
RayRelation Rays::rayIntersectAABBFaces(
	...
	if (rayIntersectAAFace<AABBFaces::KINDS_ORDER[0]>(rayOrigin, rayDir, boxFaces.faces[0].first, boxFaces.faces[0].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (rayIntersectAAFace<AABBFaces::KINDS_ORDER[1]>(rayOrigin, rayDir, boxFaces.faces[1].first, boxFaces.faces[1].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (rayIntersectAAFace<AABBFaces::KINDS_ORDER[2]>(rayOrigin, rayDir, boxFaces.faces[2].first, boxFaces.faces[2].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (rayIntersectAAFace<AABBFaces::KINDS_ORDER[3]>(rayOrigin, rayDir, boxFaces.faces[3].first, boxFaces.faces[3].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (rayIntersectAAFace<AABBFaces::KINDS_ORDER[4]>(rayOrigin, rayDir, boxFaces.faces[4].first, boxFaces.faces[4].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (rayIntersectAAFace<AABBFaces::KINDS_ORDER[5]>(rayOrigin, rayDir, boxFaces.faces[5].first, boxFaces.faces[5].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}
