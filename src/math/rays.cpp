...
RayRelation Ray::intersectAABBFaces(
	...
	if (intersectYZFace(boxFaces.faces[0].first, boxFaces.faces[0].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectYZFace(boxFaces.faces[1].first, boxFaces.faces[1].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectXZFace(boxFaces.faces[2].first, boxFaces.faces[2].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectXZFace(boxFaces.faces[3].first, boxFaces.faces[3].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectXYFace(boxFaces.faces[4].first, boxFaces.faces[4].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}

	if (intersectXYFace(boxFaces.faces[5].first, boxFaces.faces[5].second, bufNormal, faceDist) > RayRelation::None && faceDist < distance_ret) {
		isIntersect = true;
		normal_ret = bufNormal;
		distance_ret = faceDist;
	}
