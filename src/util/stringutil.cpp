...
static void base64_encode_(const ubyte* segment, char* output) {
    ...
    output[1] = ABC[((segment[0] & 0b11) << 4) | ((segment[1] & 0b11110000) >> 4)];
    output[2] = ABC[((segment[1] & 0b1111) << 2) | ((segment[2] & 0b11000000) >> 6)];
...
static std::string base64_encode_impl(const ubyte* data, size_t size) {
    ...
    if (trailing) {
        ...
        output[1] = ABC[((ending[0] & 0b11) << 4) | ((ending[1] & 0b11110000) >> 4)];
        if (trailing > 1) {
            output[2] = ABC[((ending[1] & 0b1111) << 2) | ((ending[2] & 0b11000000) >> 6)];
        }
        if (trailing > 2) {
            output[3] = ABC[ending[2] & 0b111111];
        }
        ss << output;
    }
    return ss.str();
