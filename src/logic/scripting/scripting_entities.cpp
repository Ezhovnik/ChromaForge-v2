...
static void create_component(
    ...
    if (auto runtime = content.getPackRuntime(std::string(prefix))) {
        ...
        logger.warning() << "Pack environment is not available for " << component.name;
    }
...
void scripting::on_entity_spawn(
    ...
    for (auto& component : components) {
        try {
            ...
            logger.error() << "Could not to initialize user component " << component->name << ": " << err.what();
        }
    }
...
void scripting::on_entity_player_set(const Entt_Entity& entity, int64_t pid) {
    process_entity_callback(
        entity,
        "on_player_set",
...
void scripting::on_sensor_enter(
    const Entt_Entity& entity, size_t index, entityid_t oid
) {
    process_entity_callback(
        entity,
        "on_sensor_enter",
...
void scripting::on_attacked(
    const Entt_Entity& entity, Player* player, entityid_t attacker
) {
    process_entity_callback(
        entity,
        "on_attacked",
