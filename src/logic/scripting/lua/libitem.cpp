...
static int l_item_index(lua::State* L) {
    ...
    return lua::pushinteger(L, scripting::content->requireItem(name).rt.id);
}

static int l_item_stack_size(lua::State* L) {
    auto indices = scripting::content->getIndices();
    auto id = lua::tointeger(L, 1);
    if (id < 0 || static_cast<size_t>(id) >= indices->countItemDefs()) return 0;
...
static int l_item_defs_count(lua::State* L) {
    return lua::pushinteger(L, scripting::indices->countItemDefs());
}
