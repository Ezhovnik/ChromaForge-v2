...
static const std::string& request_node_id(
    UIDocument& document, gui::UINode& node
) {
    const std::string& id = node.getId();
    if (id.empty()) {
        node.setId("#" + std::to_string(reinterpret_cast<std::ptrdiff_t>(&node)));
...
static int p_get_index_by_pos(gui::UINode*, lua::State* L) {
    return lua::pushcfunction(L, l_get_index_by_pos);
}

static int p_get_line_y(gui::UINode*, lua::State* L) {
