...
#include <logic/scripting/io_descriptors.h>
...
static int l_open_descriptor(lua::State* L) {
    ...
    if (wplusMode) {
        int temp_descriptor = scripting::io_descriptors::open_descriptor(path, false, true);

        if (temp_descriptor == -1) {
            throw std::runtime_error("Failed to open descriptor for initial reading");
        }

        auto* in_stream = scripting::io_descriptors::get_input(temp_descriptor);
        ...
        scripting::io_descriptors::close(temp_descriptor);
    }

    int descriptor = scripting::io_descriptors::open_descriptor(path, write, read);
    ...
    if (wplusMode) {
        auto* out_stream = scripting::io_descriptors::get_output(descriptor);
...
static int l_has_descriptor(lua::State* L) {
    return lua::pushboolean(L, scripting::io_descriptors::has_descriptor(lua::tointeger(L, 1)));
}
...
static int l_read_descriptor(lua::State* L) {
    ...
    auto& stream = scripting::io_descriptors::require_input(descriptor);
    if (stream.eof()) {
        stream.clear();
    }
...
static int l_write_descriptor(lua::State* L) {
    ...
    auto& stream = scripting::io_descriptors::require_output(descriptor);
...
static int l_seek_descriptor(lua::State* L) {
    ...
    if (scripting::io_descriptors::is_writeable(descriptor)) {
        auto& stream = scripting::io_descriptors::require_output(descriptor);
...
static int l_flush_descriptor(lua::State* L) {
    ...
    scripting::io_descriptors::flush(descriptor);
    return 0;
}
...
static int l_close_descriptor(lua::State* L) {
    ...
    scripting::io_descriptors::close(descriptor);
    return 0;
}

static int l_close_all_descriptors(lua::State* L) {
    scripting::io_descriptors::close_all_descriptors();
    return 0;
}
