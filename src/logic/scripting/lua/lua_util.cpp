...
int lua::pushvalue(lua::State* L, const dynamic::Value& value) {
...
dynamic::Value lua::tovalue(lua::State* L, int idx) {
    using namespace dynamic;
    ...
    switch (type) {
        ...
        default:
            std::string errLog = "Lua type " + std::string(luaL_typename(L, type)) + " is not supported";
            log_error(errLog);
