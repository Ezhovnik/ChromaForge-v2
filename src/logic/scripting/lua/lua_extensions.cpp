...
int l_debug_print(lua::State* L) {
    ...
    std::function<void(int, int, bool)> debugPrint = [&](
        int index,
        int depth,
        bool is_key)
    {
...
inline constexpr int MAX_SHORT_STRING_LEN = 50;
...
static std::string get_short_value(lua::State* L, int idx, int type) {
    switch (type) {
        ...
        case LUA_TFUNCTION: {
            std::stringstream ss;
            ss << "function: 0x" << std::hex << reinterpret_cast<ptrdiff_t>(lua::topointer(L, idx));
            return ss.str();
        }
        case LUA_TUSERDATA: {
            std::stringstream ss;
            ss << "userdata: 0x" << std::hex << reinterpret_cast<ptrdiff_t>(lua::topointer(L, idx));
            return ss.str();
        }
        case LUA_TTHREAD: {
            std::stringstream ss;
            ss << "thread: 0x" << std::hex << reinterpret_cast<ptrdiff_t>(lua::topointer(L, idx));
            return ss.str();
        }
        default: {
            std::stringstream ss;
            ss << "cdata: 0x" << std::hex << reinterpret_cast<ptrdiff_t>(lua::topointer(L, idx));
            return ss.str();
        }
    }
}
...
static dv::value create_stack_trace(lua::State* L, int initFrame = 2) {
    ...
    while (lua_getstack(L, level, &frame)) {
        ...
        if (frame.source) {
            const char* src = (frame.source[0] == '@') ? frame.source + 1 : frame.source;
            entry["source"] = src;
...
static int l_debug_pause(lua::State* L) {
    if (auto server = scripting::engine->getDebuggingServer()) {
        std::string reason;
        std::string message;
        if (lua::isstring(L, 1)) {
...
static int l_debug_sendvalue(lua::State* L) {
    auto server = scripting::engine->getDebuggingServer();
    if (!server) {
        return 0;
    }
...
static int l_debug_pull_events(lua::State* L) {
    ...
    for (int i = 0; i < events.size(); ++i) {
        ...
        if (auto dto = std::get_if<devtools::BreakpointEventDto>(&event.data)) {
            ...
            lua::rawseti(L, 3);
        } else if (auto dto = std::get_if<devtools::GetValueEventDto>(&event.data)) {
...
static int l_debug_is_debugging(lua::State* L) {
    return lua::pushboolean(L, scripting::engine->getDebuggingServer() != nullptr);
}
