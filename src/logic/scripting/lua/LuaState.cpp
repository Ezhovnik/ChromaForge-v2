...
int lua::LuaState::pushinteger(lua_Integer x) {
    lua_pushinteger(L, x);
    return 1;
}

int lua::LuaState::pushnumber(lua_Number x) {
    lua_pushnumber(L, x);
    return 1;
}

int lua::LuaState::pushivec3(lua_Integer x, lua_Integer y, lua_Integer z) {
...
int lua::LuaState::pushvalue(const dynamic::Value& value) {
    ...
    } else if (auto mapptr = std::get_if<dynamic::Map_sptr>(&value)) {
        auto map = *mapptr;
...
lua_Integer lua::LuaState::tointeger(int index) {
    return lua_tointeger(L, index);
}

lua_Number lua::LuaState::tonumber(int index) {
...
int lua::LuaState::createEnvironment(int parent) {
    ...
    if (parent == 0) lua_pushvalue(L, LUA_GLOBALSINDEX);
