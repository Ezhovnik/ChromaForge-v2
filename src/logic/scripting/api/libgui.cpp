...
int l_gui_getattr(lua_State* L) {
    ...
    if (attr == "color") return lua::pushcolor_arr(L, node->getColor());
    else if (attr == "coord") return lua::pushvec2_arr(L, node->getCoord());
    else if (attr == "size") return lua::pushvec2_arr(L, node->getSize());

    if (getattr(L, dynamic_cast<gui::Button*>(node), attr)) return 1;
    if (getattr(L, dynamic_cast<gui::Label*>(node), attr)) return 1;
