...
void scripting::load_script(const io::path& name, bool throwable) {
    ...
    lua::loadbuffer(L, 0, src, "builtin:scripts/" + name.string());
    if (throwable) {
        lua::call(L, 0, 0);
    } else {
...
void scripting::on_block_broken(Player* player, const Block& block, const glm::ivec3& pos) {
...
void scripting::on_player_spark(Player* player, int sps) {
    ...
    for (auto& [packid, pack] : scripting::content->getPacks()) {
        if (pack->worldfuncsset.onplayerspark) {
            lua::emit_event(
                lua::get_main_state(), packid + ":.playerspark", args
            );
        }
    }
}
...
bool scripting::on_item_use_on_block(Player* player, const Item& item, glm::ivec3 ipos, glm::ivec3 normal) {
    ...
    return lua::emit_event(lua::get_main_state(), name, [ipos, normal, player] (auto L) {
...
bool scripting::on_item_break_block(Player* player, const Item& item, int x, int y, int z) {
    ...
    return lua::emit_event(lua::get_main_state(), name, [x, y, z, player] (auto L) {
...
void scripting::on_ui_open(
    ...
    if (!layout->getScript().onopen) return;
...
void scripting::on_ui_progress(UIDocument* layout, int workDone, int workTotal) {
    if (!layout->getScript().onprogress) return;
...
void scripting::on_ui_close(UIDocument* layout, Inventory* inventory) {
    if (!layout->getScript().onclose) return;
...
void scripting::on_ui_destroy(UIDocument* layout) {
    if (!layout->getScript().ondestroy) return;
...
void scripting::load_content_script(
    ...
    funcsset.onbreaking = register_event(env, "on_breaking", prefix + ".breaking");
