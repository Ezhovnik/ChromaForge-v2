...
ByteReader::ByteReader(const std::vector<ubyte>& data) : data(data.data()), size(data.size()), pos(0) {
}
...
float ByteReader::getFloat32(bool bigEndian) {
    ...
    if (bigEndian) i32_val = dataio::be2h(i32_val);
    float val;
...
double ByteReader::getFloat64(bool bigEndian) {
    ...
    if (bigEndian) i64_val = dataio::be2h(i64_val);
    double val;
