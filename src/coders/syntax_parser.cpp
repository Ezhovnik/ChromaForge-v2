    ...
    std::vector<Token> tokenize() {
        ...
        while (hasNext()) {
            ...
            if (is_common_identifier_start(c)) {
                ...
                TokenTag tag = (syntax.keywords.find(name) == syntax.keywords.end() ? TokenTag::Name : TokenTag::Keyword);
                emitToken(
                    tag,
                    std::move(name),
                    start
            ...
            switch (c) {
                case '(': case '[': case '{': 
                    emitToken(TokenTag::OpenBracket, std::wstring({c}), start, true);
                    continue;
                case ')': case ']': case '}': 
                    emitToken(TokenTag::CloseBracket, std::wstring({c}), start, true);
                    continue;
                case ',':
