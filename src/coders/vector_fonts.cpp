...
static debug::Logger logger("vector-fonts");
...
namespace {
    class FTFontFile : public vector_fonts::FontFile, public std::enable_shared_from_this<FTFontFile> {
    public:
        FTFontFile(
...
void vector_fonts::initialize() {
    if (FT_Error error = FT_Init_FreeType(&library)) {
        throw std::runtime_error(
            "Could not initialize freetype: " + std::string(get_ft_error_message(error))
        );
    }
}
...
std::shared_ptr<vector_fonts::FontFile> vector_fonts::load_font(
    ...
    if (FT_Error error =
            FT_New_Memory_Face(
                library, bytes.data(), bytes.size(), 0, &face
            )
    ) {
        throw std::runtime_error(
