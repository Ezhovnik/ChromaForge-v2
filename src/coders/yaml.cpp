...
static dv::value perform_literal(std::string_view literal) {
    if (literal == "true" || literal == "True" || literal == "false" || literal == "False") {
        return literal[0] == 't';
    }
...
Parser::Parser(std::string_view filename, std::string_view source) : BasicParser(filename, source) {
    hashComment = true;
}
...
dv::value Parser::parseFullValue(int indent) {
    ...
    if (c == '\n') {
        ...
        skip(1);
        Chomping chomp = Chomping::Clip;
        if (source[pos] == '-' || source[pos] == '+') {
            chomp = source[pos] == '-' ? Chomping::Strip : Chomping::Keep;
            skip(1);
        }
        skipWhitespace(false);
        expectNewLine();
