...
Attribute Node::attr(const std::string& name, const std::string& def) const {
    auto found = attrs.find(name);
    if (found == attrs.end()) return Attribute(name, def);
    return found->second;
}
...
namespace {
    class CFModelParser : public BasicParser<char> {
    public:
        CFModelParser(std::string_view filename, std::string_view source) : BasicParser(filename, source) {
...
std::unique_ptr<Document> xml::parse_cfmodel(
    std::string_view filename, std::string_view source, std::string_view tag
) {
    CFModelParser parser(filename, source);
    return parser.parse(std::string(tag));
}

inline void newline(std::stringstream& ss, bool nice, const std::string& indentStr, int indent) {
    if (!nice) return;
    ss << '\n';
...
static void stringifyElement(std::stringstream& ss, const Node& element, bool nice, const std::string& indentStr, int indent) {
    if (element.isText()) {
