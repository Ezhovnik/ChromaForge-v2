...
public:
    explicit memory_view_streambuf(const util::Buffer<char>& buffer) : buffer(std::move(buffer)) {
...
class memory_view_istream : private memory_view_streambuf, public std::istream {
public:
    explicit memory_view_istream(const util::Buffer<char>& buffer)
        : memory_view_streambuf(buffer), std::istream(this) {}
};
