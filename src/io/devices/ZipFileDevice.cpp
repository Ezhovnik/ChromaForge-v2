...
#include <io/memory_istream.h>
#include <io/memory_ostream.h>
#include <io/deflate_istream.h>
#include <io/deflate_ostream.h>
#include <util/Buffer.h>
#include <io/io.h>
#include <coders/byte_utils.h>

static debug::Logger logger("zip-file-device");
...
namespace {
    ...
    file_time_type msdos_to_file_time(uint16_t date, uint16_t time) {
        ...
        return file_time_type::clock::now() + (time_point - std::chrono::system_clock::now());
    }
    ...
    uint32_t to_ms_dos_timestamp(const file_time_type& fileTime) {
        auto timePoint = std::chrono::time_point_cast<std::chrono::system_clock::duration>(
            fileTime - file_time_type::clock::now() + std::chrono::system_clock::now()
        );
...
ZipFileDevice::ZipFileDevice(
    ...
    separateFunc(std::move(separateFunc))
{
...
std::unique_ptr<std::istream> ZipFileDevice::read(std::string_view path) {
    ...
    if (entry.compressionMethod == COMPRESSION_NONE) {
        ...
        throw std::runtime_error("Unsupported compression method [" + std::to_string(entry.compressionMethod) + "]");
    }
}
...
public:
    ListPathsGenerator(std::vector<std::string> names) : names(std::move(names)) {};
...
static void write_headers(
    ...
    header.putInt16(10);
    header.putInt16(0);
    header.putInt16(compression_method);
    header.putInt32(timestamp);
    header.putInt32(crc);
    ...
    central_dir.putInt16(10);
    central_dir.putInt16(0);
    central_dir.putInt16(0);
    central_dir.putInt16(compression_method);
    central_dir.putInt32(timestamp);
    central_dir.putInt32(crc);
    ...
    central_dir.putInt16(0);
    central_dir.putInt16(0);
    central_dir.putInt32(0);
    central_dir.putInt32(local_header_offset);
...
void io::write_zip(const path& folder, const path& file) {
    ...
    eocd.putInt16(0);
    eocd.putInt16(0);
    eocd.putInt16(entries); 
    eocd.putInt16(entries);
    eocd.putInt32(central_dir.size());
    eocd.putInt32(central_dir_offset);
    eocd.putInt16(0);
