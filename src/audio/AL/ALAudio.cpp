    ...
    sampleRate(sampleRate)
{
    ...
    if (deviceName) {
        ...
        LOG_WARN("Could not retrieve input device specifier");
    }
}
...
duration_t ALStream::getTime() const {
    ...
    if (alspeaker) {
        ...
        if (source->isSeekable()) total %= source->getTotalSamples();
    }
...
duration_t ALSpeaker::getTime() const {
    if (stream) return stream->getTime();
...
void ALSpeaker::setTime(duration_t time) {
    if (stream) return stream->setTime(time);
...
Priority ALSpeaker::getPriority() const {
    return priority;
}
...
ALAudio::~ALAudio() {
    for (uint source : allsources) {
        ...
        if (state == AL_PLAYING || state == AL_PAUSED) AL_CHECK(alSourceStop(source));
